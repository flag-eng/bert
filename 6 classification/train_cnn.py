...
writer = SummaryWriter('runs/bert_cnn_classifier')

class BertCNNClassifier(nn.Module):
    def __init__(self, bert_model_name, num_classes, cnn_kernel_sizes=(2, 3, 4), num_filters=128, dropout=0.1):
        ...
        self.fc = nn.Linear(num_filters * len(cnn_kernel_sizes), num_classes)
    ...
    def conv_pool(self, x, conv):
        ...
        size = x.size(2)
        x = nn.functional.max_pool1d(x, size)  # shape[batch_size, out_channels, 1]
    ...
    def forward(self, input_ids, token_type_ids, attention_mask):
        ...
        last_hidden_state = outputs.last_hidden_state  # Use pooled output for classification
        # Change shape for CNN
        out = last_hidden_state.unsqueeze(1) # shape [batch_size, 1, max_len, hidden_size]
...
def val(model, processor, args, label_list, tokenizer, device, loss_fn):
    '''模型验证
    ...
    for step, (input_ids, input_mask, segment_ids, label_ids) in enumerate(tqdm(eval_dataloader, desc="valid")):
        ...
        with torch.no_grad():
            ...
            if step%100==0:
                writer.add_scalar('Validation/Accuracy', ((pred == label_ids).sum()/args.eval_batch_size).item(),step)
                writer.add_scalar('Validation/Recall', loss.item(),step)

    # 计算精确率和召回率
    correct += (predict == gt).sum()
    acc = (correct / len(gt)).item()
    loss = total_loss / step

    f1 = np.mean(metrics.f1_score(predict, gt, average=None))
    print(f'\nvalid:acc:{acc},loss:{loss},f1:{f1}\n')
    logger.info(f'\nvalid:acc:{acc},loss:{loss},f1:{f1}\n')
    logger.info(f'\nvalid:loss:{collect_loss} \n')

    return f1


def test(model, processor, args, label_list, tokenizer, device, loss_fn):
    '''模型测试
    ...
    correct += (predict == gt).sum()
    acc = (correct / len(gt)).item()
    loss = total_loss / len(gt)

    f1 = np.mean(metrics.f1_score(predict, gt, average=None))
    print(f'\ntest:acc:{acc},loss:{loss},f1:{f1}\n')
    logger.info(f'\nacc:{acc},loss:{loss},f1:{f1}\n')
...
def main():
    ...
    '''parser.add_argument("--class_weight",
                        default=[0.7221920125672164, 1.039933878545328, 1.160823540837137, 0.9599261162865403,
                                 1.724570769008801, 0.8542128206960623],
                        type=list,
                        help="类别权重值，平衡数据集")'''
    ...
    tokenizer = BertTokenizer.from_pretrained("../chinese_L-12_H-768_A-12", do_lower_case=args.do_lower_case)
    ...
    model = BertCNNClassifier("../chinese_L-12_H-768_A-12", len(label_list), num_filters=num_filters)
    ...
    '''class_weights = torch.tensor(args.class_weight)
    ...
    if args.do_train:
        ...
        for i in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                num_correct = (torch.max(logits, 1)[1] == label_ids).sum()
                acc = num_correct / args.train_batch_size
                collect_loss.append(loss.item())
                collect_acc.append(acc.item())
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if steps % 100 == 0:
                        writer.add_scalar('Training/Accuracy', acc.item(), step)
                        writer.add_scalar('Training/Recall', loss.item(), step)
                        print("batch:", steps)
                        print('Training Loss: {0:.4f}'.format(loss.item()),
                              'Training Accuracy: {0: .2f}'.format(acc.item()),
                              'learning-rate:{0:.8f}'.format(scheduler.get_last_lr()[0]))
                    steps += 1

            logger.info(f'\ntrain:epoch{i},loss:{collect_loss},acc:{collect_acc} \n')
            f1 = val(model, processor, args, label_list, tokenizer, device, loss_fn)
    ...
    test(model, processor, args, label_list, tokenizer, device, loss_fn)
    writer.close()
