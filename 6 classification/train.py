...
def main():
    ...
    '''parser.add_argument("--class_weight",
                        default=[0.7221920125672164, 1.039933878545328, 1.160823540837137, 0.9599261162865403, 1.724570769008801, 0.8542128206960623],
                        type=list,
                        help="类别权重值，平衡数据集")'''
    ...
    config = BertConfig.from_pretrained('../chinese_L-12_H-768_A-12',  num_labels=len(label_list))
    config.hidden_dropout_prob = args.dropout  # 设置dropout比率
    ...
    model = BertForSequenceClassification.from_pretrained("../chinese_L-12_H-768_A-12", config=config)

    '''BertForSequenceClassification是在 BertModel的基础上，添加了一个线性层＋激活函数，用于分类。
    而Huggingface 提供的预训练模型bert-base-uncased 只包含BertModel的权重，不包括线性层＋激活函数的权重。
    使用model = BetForSequenceClassification.from_pretrained("bert-base-uncased", config=config)来加载模型，
    那么线性层＋激活函数的权重就会随机初始化。我们的目的，就是通过微调，学习到线性层＋激活函数的权重。
    ...
    #BertForSequenceClassification 里面已经有了一个 CrossEntropyLoss()，重定义损失函数平衡数据集
    ...
    if args.do_train:
        ...
        for i in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                num_correct = (torch.max(outputs[1], 1)[1] == label_ids).sum()
